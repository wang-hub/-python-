# -*- coding: utf-8 -*-
"""
Created on Mon May 25 15:21:05 2020
...
def get_suffixfiles_fulloppath(path,suffix):
    """
    获取指定路径下所有.xxx文件的路径
    因为用于html，中间用/连接
    """
    files = list(filter(lambda x: x.endswith(
        suffix), os.listdir(path)))
    FilesFullPath = list(map(lambda x: path + '/' + x, files))
    #print(FilesFullPath)
    return natsort.natsorted(FilesFullPath)
...
if __name__ == '__main__':
    pass
    #img_dir='jingying_img'
    #get_suffixfiles_fulloppath(img_dir,'.jpg')